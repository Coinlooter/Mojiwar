alter type public.card_rarity add value if not exists 'legendary';

alter type public.card_effect_type add value if not exists 'vampiric_lifesteal';
alter type public.card_effect_type add value if not exists 'thorns_reflect';
alter type public.card_effect_type add value if not exists 'battle_frenzy_attack';
alter type public.card_effect_type add value if not exists 'opening_barrier';
alter type public.card_effect_type add value if not exists 'double_strike_chance';

alter table public.player_cards
  add column if not exists quality public.card_rarity,
...
begin
  ...
  if p_reward_card_roll is not null and p_reward_talisman_id is not null then
